...
The server will create a named pipe and will wait for a request to sent over thenamed pipe. The user request will have the form RANGE XX YY, where XX is thelower bound and YY is the upper bound. The server will then proceed to compute all primes in that range and return them to the process that made the request. If therequest is malformed, the server will return a "malformed request" message. requestscan come from any number of processes.
